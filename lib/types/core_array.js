    ...
    if (parent) {
      ...
      if (arguments.length) {
        if (embeddedPath != null) {
          ...
          // HACK TO HANDLE NESTED DOC ARRAYS
          const realArray = parent[this.$path()];
          if (!realArray || realArray === this) {
            dirtyPath = dirtyPath + '.' + this.indexOf(elem) + '.' + embeddedPath;
          } else {
            const nestedIdx = realArray.indexOf(this);
            dirtyPath = dirtyPath + '.' + nestedIdx + '.' + this.indexOf(elem) + '.' + embeddedPath;
          }
